...
function createTunFromFd (fd, name, opts) {
  ...
  function readLoop () {
    ...
    fs.read(fd, buf, 0, buf.length, -1, function (err, n) {
      if (released) return
      if (err) {
        // EBADF = fd not valid (e.g. not inherited by child process)
        // Don't retry — release and let the caller know
        if (err.code === 'EBADF') {
          console.error('TUN fd ' + fd + ' is not valid (EBADF)')
          tun.release()
          return
        }
        if (tun.listenerCount('error') > 0) {
          tun.emit('error', err)
        } else {
