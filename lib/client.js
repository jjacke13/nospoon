...
async function startClient ({ server, ip = '10.0.0.2/24', ipv6, seed, mtu = 1400, fullTunnel, keepalive = 25000, tun: externalTun, onConnected, ipcWrite }) {
  ...
  function connect () {
    ...
    connection.on('open', function () {
      ...
      // Full-tunnel route setup: only on desktop (Linux/macOS/Windows).
      // On Android (--fd-socket mode), Kotlin's VpnService.Builder handles
      // routing via addRoute("0.0.0.0", 0) — the binary must NOT touch routes.
      if (fullTunnel && !ipcWrite) {
