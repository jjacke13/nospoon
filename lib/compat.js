...
// Directory of the running executable (for finding bundled files like wintun.dll)
exports.execDir = isBare
  ? (isBare ? require('bare-path') : require('path')).dirname(Bare.argv[0])
  : require('path').dirname(process.execPath)
// Bare doesn't have process.env — use bare-os.getEnv() behind a Proxy
if (isBare) {
  const bareOs = require('bare-os')
  exports.env = new Proxy({}, {
    get: function (_, key) { return bareOs.getEnv(key) },
    has: function (_, key) { return bareOs.hasEnv(key) }
  })
} else {
