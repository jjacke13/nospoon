...
const { randomBytes, exit, onSignal } = require('./compat')
...
async function startServer ({ ip = '10.0.0.1/24', ipv6, seed, mtu = 1400, peers, fullTunnel, outInterface, keepalive = 25000, tun: externalTun }) {
  const seedBuf = seed
    ? b4a.from(seed, 'hex')
    : randomBytes(32)
  ...
  const dht = new HyperDHT({ connectionKeepAlive: keepalive })
  const tun = externalTun || createTunDevice({ ipv4: ip, ipv6, mtu })
