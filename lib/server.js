...
async function startServer ({ ip = '10.0.0.1/24', ipv6, seed, mtu = 1400, config, fullTunnel, outInterface }) {
  ...
  const server = dht.createServer(function (connection) {
    const clientKeyHex = connection.remotePublicKey.toString('hex')
    const clientKeyShort = clientKeyHex.slice(0, 8) + '...'

    // Check if client is allowed
    if (allowedPeers && !allowedPeers.has(clientKeyHex)) {
      console.log(`Rejected unknown client: ${clientKeyShort}`)
      connection.destroy()
      return
    }

    const clientIp = allowedPeers
    ...
    const decode = createDecoder(function (packet) {
      // Verify source IP matches assigned IP (prevent spoofing)
      if (clientIp) {
