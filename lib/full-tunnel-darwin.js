const { fs, path, os } = require('./compat')
const { validateInterface, run } = require('./platform-utils')
...
const { TUNNEL_DNS } = require('./platform-utils')
...
function enableClientFullTunnel (remoteHost, tunName) {
  ...
  run('route', ['add', '-inet6', '-net', '::/1', '-interface', tun])
  run('route', ['add', '-inet6', '-net', '8000::/1', '-interface', tun])
