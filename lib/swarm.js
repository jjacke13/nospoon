...
async function startSwarm (opts) {
  ...
  let { onConnected } = opts

  // Identity — seed is always in config
  const seedBuf = b4a.from(opts.seed, 'hex')
  ...
  swarm.on('connection', function (connection) {
    ...
    // Authentication — reject unknown peers
    if (!peers.has(remoteKeyHex)) {
      console.log(`Rejected unknown peer: ${remoteKeyShort}`)
      connection.destroy()
      return
    }

    const peerIp = peers.get(remoteKeyHex)
  ...
  console.log('Peers:        ', peers.size - 1, 'other')
  for (const [key, pIp] of peers) {
    const isSelf = key === keyPair.publicKey.toString('hex')
    console.log(`  ${key.slice(0, 8)}... → ${pIp}${isSelf ? ' (you)' : ''}`)
  }
