...
// Filter the TUN's address out of the set of "local addresses" that
// HyperDHT announces to peers in its handshake replies.  By default the
// library calls `socket.udx.networkInterfaces()` and announces every
// non-loopback IPv4 interface — which includes nospoon's own TUN
// (10.0.0.1).  hyperdht-cpp clients connecting from a carrier-CGNAT
// 10.x address misinterpret 10.0.0.1 as a same-LAN candidate (a /8
// prefix-match bug in hyperdht-cpp's localConnection / "LAN shortcut"
// path) and time out trying to reach it directly.  Stripping the TUN
// address at the source avoids the bait regardless of which client
// implementation connects.
//
// Module-level monkey-patch: nospoon owns the process, so this does
// not affect anything else.
const _origLocalAddresses = Holepuncher.localAddresses
function patchAnnouncedAddresses (tunIp) {
  Holepuncher.localAddresses = function (socket) {
    return _origLocalAddresses(socket).filter(a => a.host !== tunIp)
  }
}
...
async function startServer ({ ip = '10.0.0.1/24', ipv6, seed, mtu = 1400, peers, fullTunnel, outInterface }) {
  ...
  // Suppress the TUN's address from DHT announcements before HyperDHT
  // is constructed — see the comment on patchAnnouncedAddresses above.
  // (`tunIp` is re-derived for logging further down.)
  patchAnnouncedAddresses(ip.split('/')[0])
